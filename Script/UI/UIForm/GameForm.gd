class_name GameForm extends UIForm

# 状态Icon预制件
@export var State_Icon_Perfab:PackedScene
# 移动端装备预制件
@export var Pc_Equip_Perfab:PackedScene
#区域名
...
#状态组
@export var PlayerStatesGrid:GridContainer
#装备列表
...
# 快捷键名称组
@export var shortcutKeyNames:Array[Label]
# 显示飞点按钮
...
func refreshShortccutKeys():
	shortcutKeyNames[0].text = MotaSystem.keysettingconfig.get_key_name("call_handbook")
	shortcutKeyNames[1].text = MotaSystem.keysettingconfig.get_key_name("call_equip")
	shortcutKeyNames[2].text = MotaSystem.keysettingconfig.get_key_name("call_minimap")
	shortcutKeyNames[3].text = MotaSystem.keysettingconfig.get_key_name("call_shop")
	shortcutKeyNames[4].text = MotaSystem.keysettingconfig.get_key_name("call_save")	
	shortcutKeyNames[5].text = MotaSystem.keysettingconfig.get_key_name("call_load")
	shortcutKeyNames[6].text = MotaSystem.keysettingconfig.get_key_name("auto_load")
	shortcutKeyNames[7].text = MotaSystem.keysettingconfig.get_key_name("call_menu")	
	shortcutKeyNames[0].tooltip_text = MotaSystem.keysettingconfig.get_key_name("call_handbook")	
	shortcutKeyNames[1].tooltip_text = MotaSystem.keysettingconfig.get_key_name("call_equip")
	shortcutKeyNames[2].tooltip_text = MotaSystem.keysettingconfig.get_key_name("call_minimap")
	shortcutKeyNames[3].tooltip_text = MotaSystem.keysettingconfig.get_key_name("call_shop")
	shortcutKeyNames[4].tooltip_text = MotaSystem.keysettingconfig.get_key_name("call_save")	
	shortcutKeyNames[5].tooltip_text = MotaSystem.keysettingconfig.get_key_name("call_load")
	shortcutKeyNames[6].tooltip_text = MotaSystem.keysettingconfig.get_key_name("auto_load")
	shortcutKeyNames[7].tooltip_text = MotaSystem.keysettingconfig.get_key_name("call_menu")	
	# 根据实际文本长度调节字体大小
	for i in shortcutKeyNames:
		if i.text.length() > 2:
			i.label_settings.font_size = 18
		else:
			i.label_settings.font_size = 25
...
func creatEquipUI():
	...
	for i in MotaSystem.gameData.equip_slot:
		...
		var equiplist_child = Pc_Equip_Perfab.instantiate()
...
func onShowFlyPointClick():
	if !MotaSystem.gameVariables["showFlyPoint"]:	
